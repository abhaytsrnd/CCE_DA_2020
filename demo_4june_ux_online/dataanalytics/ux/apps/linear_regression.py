...
def linear_regression(n):
    ...
    html.Div([
        ...
        html.H2('r threshold: ' + str(round(1.96/sqrt(n),4))),
...
def stats_table_and_linear_regression(json_ordered_data):
    ...
    for i in range(len(x_col)):
        ...
        data[i] = x
    y = list(dff[col[-1]])
    ##Online Team 3 API Integration
    try:
        model = Team3onlineLinearRegression(data, y)
        db.put("lr.model", model)
        db.put("lr.x_col", x_col)
        db.put("lr.y_col", y_col)
        (params, summary, ycap) = model.fit()
...
def lr_predict_data(n_clicks):
    ...
    predicted = model.predicts(predict_data)
